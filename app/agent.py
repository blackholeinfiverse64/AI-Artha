...
class RLAgent:
    ...
    def metrics(self) -> Dict:
        ...
        try:
            ...
            db_content_count = get_collection("content").count_documents({})
            db_feedback_count = get_collection("feedback").count_documents({})
        except Exception:
    ...
    def sync_with_database(self):
        ...
        try:
            from core.mongodb import get_collection
            rows = get_collection("content").find().sort("uploaded_at", -1).limit(500)
            
            for doc in rows:
                content_id = doc.get("content_id")
                tags_json = doc.get("current_tags", "")
                authenticity = doc.get("authenticity_score")
                try:
